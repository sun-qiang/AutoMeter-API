...
//        TestAssert.isCaseresult()
        try {
            ...
            boolean apiresult = TestAssert.isCaseresult();
            AssertInfo = FixDBAssert(AssertInfo, TestAssert, requestObject.getApicasesDBAssertList());
            boolean dbresult = TestAssert.isCaseresult();
            if (apiresult == true && dbresult == true) {
                TestAssert.setCaseresult(true);
            } else {
                TestAssert.setCaseresult(false);
            }
                    ...
                    if (machinelist.size() == 0) {
                        ...
                        if (AssembleType.equalsIgnoreCase("pgsql")) {
                            ...
                            if (AssembleType.equalsIgnoreCase("金仓")) {
                                ...
                                logger.info(" 开始处理金仓initDbResource。。。。。。。。。。。。。1111 ");
